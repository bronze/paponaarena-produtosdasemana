...
- Produtos com `parentId` agregam menções ao produto pai (ex: "Uber Black" → pai "Uber", "Google AI Studio" → pai "Gemini"). Produtos filhos não aparecem em leaderboards nem em totais de categoria — suas menções são somadas ao pai
- Produtos combo têm `alsoCredits: string[]` e representam menções que cobrem múltiplos produtos simultaneamente (ex: `{ id: "uber-livelo", name: "Uber + Livelo", alsoCredits: ["uber", "livelo"] }`). Outros exemplos reais: "Cursor + Claude Code", "Gemini + Vercel", "Buscapé + Zoom", "Caneta + Caderno", "Mosaico do Youtube na CazéTV" (`alsoCredits: ["youtube", "cazetv"]`), "Zap Imóveis + Quinto Andar + Viva Real + Net Imóveis" (combo com 4 produtos). Combos também são excluídos de leaderboards e totais de categoria — cada produto em `alsoCredits` recebe +1 menção individualmente quando o combo é citado
...
Abaixo os dados dos episodios que temos ate hoje:
